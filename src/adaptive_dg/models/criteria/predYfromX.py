...
class PredYfromX(BaseModel):
    ...
    def compute_metrics(self, batch, batch_idx):
        ...
        y_pred_standard = self.predictor(feature_standard)
        ...
        out_dict['loss_standard'] = loss#.item()

        if self.hparams.task == "classification":
            out_dict['acc_standard'] = (y_pred_standard.argmax(1) == y.argmax(1)).float().mean().item()
            out_dict['ece_standard'] = self.ece_loss(y_pred_standard, y.argmax(1)).item()

            out_dict['acc'] = out_dict['acc_standard']
            out_dict['ece'] = out_dict['ece_standard']
    ...
    def predict(self, x, x_set=None, e=None):
        ...
        assert x_set == None, "set input must not be provided for this model"

        feature_standard = self.feature_extractor(x)
        y_pred = self.predictor(feature_standard)
        return y_pred
